...
questions = [
    {
        ...
        "answer": "D",
        "explanation": "Amazon RDS の Multi-AZ 構成を有効にすると、別の AZ にスタンバイインスタンスが自動的に作成され、データが同期的にレプリケートされます。プライマリインスタンスの障害やメンテナンス時に、AWS が自動的にフェイルオーバーを行うため、ダウンタイムを最小限に抑えることができます。\n\n*   **Cについて**: リードレプリカは主に読み取り負荷の分散に使用されます。障害時の昇格は手動またはスクリプトによる操作が必要であり、Multi-AZ ほどの自動フェイルオーバー性能はありません。\n*   **Bについて**: バックアップからの復元は時間がかかり、RTO（目標復旧時間）と RPO（目標復旧時点）の要件を満たせない可能性が高いです。\n*   **Aについて**: EC2 自管理の MySQL は管理負荷が高く、RDS の Multi-AZ のような自動化された高可用性を実現するには複雑な設定が必要です。"
    },
    {
        "id": "q02",
        ...
        "answer": "B",
        "explanation": "S3 Standard-IA は、アクセス頻度は低いものの、必要時に即座にアクセスできるデータに最適です。S3 Standard よりもストレージ料金が安く、取り出し料金が発生します。\n\n*   **Cについて**: S3 Standard は頻繁にアクセスされるデータ向けであり、月1回のアクセス頻度ではストレージ料金が割高になります。\n*   **Dについて**: S3 Glacier Flexible Retrieval はアーカイブ向けであり、データの取り出しに数分から数時間かかるため、「即座に利用可能」という要件を満たしません。\n*   **Aについて**: S3 One Zone-IA は単一の AZ に保存するため、他のクラスに比べて耐久性（アベイラビリティーゾーンの消失に対する耐性）が低く、重要なデータの長期保存には不向きな場合があります。"
    },
    {
        "id": "q03",
    ...
    },
    {
        ...
        "answer": "D",
        "explanation": "AWS Direct Connect は、オンプレミスから AWS への専用ネットワーク接続を確立するサービスです。インターネットを経由しないため、安定した広帯域（1Gbps、10Gbps 等）を確保でき、一貫したネットワーク体験が提供されます。\n\n*   **Bについて**: Site-to-Site VPN はインターネット経由で暗号化されたトンネルを構築します。セットアップは迅速ですが、インターネットの混雑状況により帯域が不安定になることがあります。\n*   **Cについて**: Client VPN は個々のクライアントデバイスから AWS への接続に使用されます。\n*   **Aについて**: CloudFront はコンテンツ配信ネットワーク（CDN）であり、ハイブリッド接続のためのサービスではありません。"
    },
    {
        "id": "q06",
    ...
    },
    {
        ...
        "answer": "A",
        "explanation": "Amazon Aurora Global Database は、1 つのプライマリリージョンと最大 5 つのセカンダリ読み取り専用リージョンをサポートします。リージョン間のレプリケーションは通常 1 秒未満であり、リージョン全体の障害が発生した場合でも 1 分未満でのフェイルオーバー（昇格）が可能です。\n\n*   **Cについて**: RDS のクロスリージョンレプリケーションは可能ですが、Aurora Global Database ほどパフォーマンスやフェイルオーバー時間は最適化されていません。\n*   **Dについて**: DynamoDB は NoSQL データベースです。要件がリレーショナルデータベース（Aurora の文脈）である場合、Aurora が適切です。\n*   **Bについて**: アプリケーション側での同期は非常に複雑で、整合性の維持が困難です。"
    },
    {
        "id": "q09",
    ...
    },
    {
        ...
        "answer": "D",
        "explanation": "AWS Config は、AWS リソースの設定を評価、監査、審査するためのサービスです。リソースの設定変更を記録し、定義したルール（Config ルール）に従ってコンプライアンス状態を評価します。\n\n*   **Cについて**: CloudTrail は「誰が・いつ・何をしたか」という API コールのログを記録しますが、リソースの設定状態の評価は行いません。\n*   **Aについて**: CloudWatch はパフォーマンスメトリクスやログの監視に使用されます。\n*   **Bについて**: Trusted Advisor はコスト削減やセキュリティのベストプラクティスをアドバイスしますが、詳細な設定変更履歴の追跡やカスタムルールの評価は Config が専門です。"
    },
    {
        "id": "q22",
